...
var LPLOCALCONTAINERCOOKIE = 'HumanClickSiteContainerID_' + lpNumber;
...
var hcImageURL = lpFixProtocol("https://server.iad.liveperson.net/hcp/liveperson/1/en/");
...
function lpParseLocalContainer() {
    return lpFindCookie(LPLOCALCONTAINERCOOKIE);
}
...
function lpSetFPCookies(id, skey, contid) {
    var d = new Date();
    ...
    if(contid != "-")
        lpSetOneFPCookie(LPLOCALCONTAINERCOOKIE,contid,null);
    lpFPCookieSet = true;
}
...
function lpAppendVisitorCookies(url) {
    ...
    var siteContainerParam = lpParseLocalContainer();
    if(siteContainerParam.length>0) {
        siteContainerParam = '&siteContainer=' + siteContainerParam;
    }

    if (index == -1) {
				var needsAmp = url.indexOf("&") != 0;
        url = '&visitor=' + lpParseLocalVisitorID() + '&msessionkey=' + lpParseLocalSessionKey() + siteContainerParam + ( needsAmp ? "&" : "") + url;
    } else {
        var firstChunk = url.substring(0, index + 1);
        var secondChunk = url.substring(index + 1);
				var needsAmp = secondChunk.indexOf("&") != 0;
        url = firstChunk + 'visitor=' + lpParseLocalVisitorID() + '&msessionkey=' + lpParseLocalSessionKey() + siteContainerParam + ( needsAmp ? "&" : "") + secondChunk;
    }
    return url;
}
...
function lpVerifyFPCookie() {
    if (! lpFPCookieSet) {
        if (lpFetchScriptCounter-- <= 0) {
            ...
            var tagSrc = hcBase + "?site=" + lpNumber + "&cmd=fetchVisitorID&callbackFunc=lpSetFPCookies('~id~', '~skey~', '~contid~');&d=" + hcDate();
...
function hcHandleWidthHeight(w,h)
{
    ...
    if (h==1) {
		...
		hcTimeout = 30;
            ...
            //change container id (FPC) to primary
            if(lpFPCookieSet) {
                lpSetOneFPCookie(LPLOCALCONTAINERCOOKIE,'primary',null);
		}
            //todo: send inPage now
            //hcCounter=0;
            hcTimeout = 0;
            console.log("image 160 response, set timeout to 0");
...
function openCredit()
{
	document.location = lpFixProtocol("http://solutions.liveperson.com/ref/lppb.asp");
}
...
function openEngageChat(param, openVars) {
	...
	if ((! lpForcePopup) && (hcIE || hcNS || hcDOM)) {
        ...
        url = lpAppendVisitorCookies(url);
		window.open(url, 'wanttochat'+lpNumber, 'width=175,height=109,menubar=no,scrollbars=no');
	}
}
